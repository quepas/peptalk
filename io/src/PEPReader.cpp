...
namespace peptalk::io {
    ...
    void PEPReader::CheckMagicNumbers() {
        ...
        in_file.read((char *) format_name.c_str(), PEP_FORMAT_TAG.size() );
        if (format_name != PEP_FORMAT_TAG) {
            throw InvalidFormat("Not a PEP format at the beginning.");
        }
        in_file.read((char *) &pep_format_version, sizeof(unsigned int));
    }

    void PEPReader::ReadNumProfiles() {
        in_file.read((char *) &num_profiles, sizeof(unsigned short int));
    }
    ...
    unsigned int PEPReader::ReadNumMeasurements() {
        ...
        in_file.read((char *) &num_measurements, sizeof(unsigned int));
